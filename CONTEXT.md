...
### Multi-replica rule (read before designing anything) [STABLE]

late.sh runs as one SSH replica today, and a multi-replica rework is coming. Every feature built from now on is designed as if several `late-ssh` processes were already running behind sticky sessions, so the rework is a migration of old debt, not a rewrite of new work. The rule, in order of strictness:

- **Truth lives in Postgres.** Nothing process-global in memory is a source of truth: no `Arc<Mutex<..>>` registries, no `AtomicBool` kill switches, no per-process counters or daily caps that another replica cannot see. A per-session mirror of DB state (read once at bootstrap, updated on the same events that write the DB) is fine: that is a cache, and the tick path must read local memory only anyway (§2.5).
- **Once-only actions are claimed in the DB, never decided in memory.** Anything that must happen exactly once per user or per world (send a DM, pay out, fire a nonrenewable beat, call a paid API) wins a conditional `UPDATE ... WHERE <not yet> RETURNING` or a `pg_advisory_xact_lock` first and acts only on the win. Existing shapes to copy: `User::claim_first_contact_invitation`, `Pot::lock_open`, the crown take, `GamePayout::grant_multi`.
- **Cross-session fan-out goes through `LISTEN/NOTIFY`,** not through a process-local `broadcast`. Shape to copy: `shop_user_changed`, `pot_changed`, `crown_changed` (a listener task seeds a process-shared `watch`, sessions read the `watch`).
...
| `late-ssh/src/app/bonsai/CONTEXT.md` | The bonsai branch graph, care modal, sidebar panel, profile hero preview, growth simulation, daily chips, badge scoring, or the Bonsai Decay Shield. | Bonsai persistence, renderers, input model, growth/death rules, the once-per-day chip gate, chat badge scoring, and invariants. |
| `late-ssh/src/app/lobby/CONTEXT.md` | The Lobby: the `Ctrl+G` modal and `LobbyState`/`LobbyEntry` over the two game domains. | Lobby domain overview and the modal contracts; routes onward to `lobby/daily/CONTEXT.md` and `lobby/house/CONTEXT.md`. |
| `late-ssh/src/app/workspace/CONTEXT.md` | The backtick workspace cycle spanning daily boards, house tables, unfinished Arcade dailies, live (detached) roguelike door games, a recently-detached Lateania world, and the still-loaded native remakes (Dark Room, Green Dragon). | The cycle's stop roster, ordering, and per-leg liveness contracts. |
...
| `late-ssh/src/app/artboard/CONTEXT.md` | Shared ASCII Artboard, dartboard code, editor input/rendering, canvas persistence, provenance, archive snapshots, artboard bans, or the gallery (the page rail, framing and hanging pieces, applause, the monthly `artboard` award and its chip prize, the splash piece, the paper's wall column). | Artboard lifecycle, live `dartboard_local` server, per-session editor state, active/view/archive input routing, swatches/glyph picker, provenance, persistence/archive rollovers, gallery contract, tests, and fragile layout/provenance areas. |
...
| `late-ssh/src/app/deadchannel/CONTEXT.md` | The deadchannel game domain (design docs `GAME.md` and `DIGEST.md` live in that directory): the first-contact haunting ladder (clock glitch, name flicker, splash whisper, invitation DM), the eligibility gate (tenure, touched settings, AI-screened bio), the glyph alphabet, `/haunt` admin controls, the haunt kill switch and fuse (`app_flags`), or the runner and its look (`deadchannel_runners`, the piece table, the #deadchannel portrait gutter). | Domain layout (game root + `haunt/` onboarding subdomain + `runner/` character layer), the four stage machines and their chain (hits arm the whisper, the whisper schedules the invitation), the gate and its thresholds, the replica rule as applied (row claims for every cap and stamp, flags through `app/flags`, looks through the `deadchannel_runner_changed` directory), persisted `first_contact_*` marks, the runner row and the look JSON contract, the `afterglow` ghost voice, render hooks, `/haunt` command surface, tests, and gotchas. |
...
| `late-ssh/src/app/zen/CONTEXT.md` | Zen (`Ctrl+F` from any page, the same chord back): Rice, the per-account tiling layout of the bonsai, tank, pet, the selected room's chat, music, and a clock. | Tile tree and persistence (`users.settings.zen_layout`), the key map, how the aquarium sim is re-bound to whichever page shows it. |
...
| `late-ssh/src/app/nightcap/CONTEXT.md` | Nightcap, the small bar out back of the Clubhouse (`n` to enter, Esc to leave): six sittable seats and a round of drinks, `Screen::Nightcap`. | The process-global `SharedSeats` contract (single-replica, same as the Clubhouse), why drinks are cosmetic for now, and the module map. |
...
| `late-ssh/src/app/profile_modal/CONTEXT.md` | The read-only profile modal (`/profile`, `/chips`, `p` on an author): the one scrolling layout, the bonsai-as-logo hero with the late.fetch grid, badges, the aquarium band, the public chips ledger. | Off-screen composition and blit, section order, the hero reflow, the `/chips` jump, the ledger row copy in `ledger.rs`, tests. |
| `late-ssh/src/app/pot/CONTEXT.md` | The weekly pot: `/pot` and `/pot buy N`, `PotService`, the `pots` / `pot_tickets` tables, the status HUD badge, the draw sweeper, or the pot's #lounge lines. | Pot domain context: the fixed numbers (100-chip tickets, 10 per player per UTC day, 80% payout, Monday 21:00 UTC), the two `ChipMove` variants (both count on Top Chips), the three concurrency guards (partial unique index, advisory lock, status transition), the `pot_changed` distribution model and the private holdings map, the status HUD badge (there is no sidebar panel any more), the draw's feed line and headline, telemetry, and the gotchas. |
| `late-ssh/src/app/paper/CONTEXT.md` | The Late Edition: `/paper`, the once-a-day login pop, the sweeper that prints one column per public room, the Elsewhere / What we were reading / Outside pages, the `paper_*` app flags, or the `paper_at_login` tweak. | Press versus newsstand split, the row-claim print contract (`printing` / `ready` / `quiet`, stale reclaim), which rooms qualify, the layout order (rail order, favorites first), the login claim, telemetry, tests, and gotchas. |
...
- **Tool bootstrap:** `.mise.toml` pins `rust`, `mold`, and `cargo-nextest`. Run `mise install` before local development so the expected toolchain and test runner are available.
- **Cargo environment setup:** For local host development, use Cargo's normal defaults, including the standard repo-local `target/` directory. Docker/dev containers use `/app/target` via container configuration. `CARGO_HOME=$HOME/.cargo` is a valid override when an environment needs it, not a repo-wide requirement.
...
- `ShopService` (in `app/hub/shop/svc.rs`) exposes per-user `watch::Receiver<ShopSnapshot>` values and purchase result broadcasts. It loads marketplace items, user purchases, and chip balance into a per-user snapshot at session init and after changes; render/input gates read the snapshot instead of querying DB on every keypress. It also runs a Postgres LISTEN/NOTIFY listener for `shop_user_changed`, `shop_catalog_changed`, and `chip_user_changed`, so multiple SSH replicas can refresh active users after another process changes shop or chip state. It additionally owns the name-flair pipeline: a process-shared `NameFlairDirectory` (`app/common/username_effect.rs`, snapshot-swap like `UsernameDirectory`: readers clone an `Arc`, no polling task) seeded once at startup and refreshed per-user from the `shop_user_changed` notify and after a local purchase. Each entry holds a color effect and a rented title as independent halves with their own expiries, plus a burn milestone, which is the one member with no expiry at all (a permanent `user_purchases` row, `MilestoneBadge::highest_for_user`, so `refresh_user_flair` has to re-read it or a rental purchase would drop it). Effect rows are user-scoped `shop_consumable_effects` (`room_id IS NULL`, `ends_at = now + duration_secs`, expiry read-time only). Catalog: 24h Name Glow 200 / Name Gradient 500 / Name Shimmer 1000, a 30-day tier of each at 40x the day price, and 36 curated `title_rental` titles at 200/24h and 6,000/30d. One active row per user per `effect_kind`: any rebuy replaces the live row and resets the clock. Each session resolves the directory into `App.name_flair` (`HashMap<Uuid, ResolvedName>`) once a second in `tick.rs` (which also steps shimmer at 1 Hz); chat author labels and clubhouse name labels paint the resolved fg per character over the bare username and print `, <title>` after it in the dim label color (effect fg overrides own-amber/friend-gold; IRC stays plain text, titles included). Purchases announce via `ActivityKind::UsernameEffectApplied` / `TitleApplied` / `BadgeRented` / `BurnMilestone` through the standard #lounge feed ("mat is glowing (24h)", "mira is now the night clerk (30d)", "mira rented 🐱 (24h)"), each repeat-keyed on what is now visible. The custom title tiers (`title_custom_day` 2,000 / `title_custom_month` 80,000) are the same `title_rental` kind and the same effect row, but the payload carries `custom: true` and no `text`, so the buyer types it into a Shop prompt. Nothing reaches the chip ledger until `CustomTitle::parse` (`late-core/src/models/rental.rs`: 20 characters, whitespace collapsed, no control/zero-width/bidi characters, no `@`) and `late-ssh/src/app/ai/screen.rs` (ungrounded schema-enforced verdict, fails closed on an unreadable or absent reply) have both passed, and `purchase_item_by_sku_with_custom_title` re-checks the SKU/text pairing inside the transaction. `ShopSnapshot.custom_titles_available` mirrors `AiService::is_enabled()`, so an install with no AI shows the SKUs as unavailable rather than selling unscreened text.

Chat badges and flags are rentals too, but they ride the chat label query rather than the flair directory: every badge has a 24h and a 30-day `badge_rental` SKU (100/3,000 basic, 250/7,500 premium, flags at the basic price), and the permanent badge SKUs are retired with `active = false`. A rental writes a user-scoped effect row under `effect_kind = 'chat_badge'` / `'chat_flag'`, and `User::list_chat_author_metadata` reads that row and nothing else, so a badge or a flag is rented or absent, never owned. `late-core/src/models/rental.rs` is the one place the two rental windows, their copy, and the payload readers live, for all four rental kinds.
...
- `Hub` (in `app/hub`) is the Shop modal, a single Shop tab titled " Shop ". It has no global chord; the `/shop` composer command (`open_shop_modal_globally`) and the locked pet/aquarium nudges open it. The hub module owns the leaderboard/quest/shop services even though leaderboards render on page `6` and quests render on The Arcade. Hub-owned marketplace and entitlement projection code lives under `app/hub/shop`. Detailed behavior lives in `late-ssh/src/app/hub/CONTEXT.md`.
- `ChipService` (in `app/games/chips/svc.rs`) manages the Late Chips economy: every chip mutation names a `ChipMove` variant (`late-core/src/models/chips.rs`, the closed roster of ledger reasons, floor guards, and earnings flags; `UserChips::apply` is the single delta write path). **Every ledger row says what it paid for**: `UserChips::apply` takes a required `source_ref` (a round or hand id for the house tables, the counterparty for gifts, the message for gilds, the date for the bonsai drip, the visit for Super Snake, and so on per variant), and the starting stipend is written by `UserChips::ensure` as `ChipMove::InitialBalance` in the same statement that creates the row, so a player's ledger sums to their balance. The one exception, by decision, is the admin-only `/grant` chat command (`UserChips::admin_grant`): no ledger row at all. **Top Chips ranks what a player earned** (`ChipMove::counts_as_earnings`): only credits count, minus the house tables (bets, payouts, floor restores; a colluding table cannot fold one seat to the top), gifts (nobody can be funnelled up the board) and the starting stipend; gilds received, the pot, the bonsai drip and every prize count, and no debit ever counts, so spending never lowers a place. The the `chip_user_changed` notify is owned by `user_chips` triggers, so no write path can forget it. `ensure_chips(user_id)` creates new chip rows with 1000 chips, its activity reward task awards daily puzzle base chips from `reward_templates` after `GameWon` events (including Sliding Puzzle's 100/250/500 easy/medium/hard tiers), and reward-template payout helpers record minted game rewards in `game_payout_claims`. Chip-paying room games and Lateania hold a `ChipService` clone. **Daily correspondence wins sit behind two gates** (`DailyService::pay_winner`): at least 5 half-moves played, and one paid win per opponent per game per UTC day the match was posted (`ChipService::credit_per_event_pair_day_reward_template`, a `pair_day` key beside the match key in one `grant_multi`, scoped to the roster game by decision so friends who play several games together are never touched), so a two-account post-claim-resign loop is worth one win per pair per game per day. **Every door milestone pays again**: the badge stays once per account, the chips repeat, and the gate is whatever naturally limits the game. The roguelike door pairs — NetHack's Amulet of Yendor (20,000 chips) and ascension (40,000), DCSS's Orb pickup (20,000) and win (40,000), Brogue's escape (20,000) and mastery (40,000), granted by the door log pipe's shared award sink (`app/door/ingest/award.rs`) from the hosts' spoof-proof log files — pay once per ingested log line and at most once every 7 days per milestone per account. Lateania's four crowns (the Archdemon Mal'gareth and the King Who Was Promised Nothing at 10,000, Yssgar the Sundering Deep and Kaethyr Ascendant at 20,000) pay once per `mud_characters.id` behind the same 7-day lockout: the character persists, so a maxed one would take the easy crowns nightly, and `d` deletes it, so a per-character gate alone would be a reroll farm. Green Dragon's dragon kill (10,000) pays for every kill, keyed on `<character row id>:<kill number>`, because the kill resets the character to level 1 and the daily turn cap makes the climb back 7-10 days. A Dark Room's two endings (`app/door/darkroom/svc.rs::reward_escape`: `ADE` for the ascent won, 15,000; `ADB` for the ascent won holding the fleet beacon off the ravaged battleship, 20,000, claimed separately) pay for every run that gets out, keyed on the `Game.run_id` in the save blob, because the ending deletes the save and a repeat is the whole arc again. The two-gate payouts run through `ChipService::credit_run_cooldown_reward_template` over `GamePayout::grant_multi`, an all-or-nothing multi-key grant under one advisory lock: any gate refusing writes no claim row, no chips, and no ledger line. `period_kind = 'lifetime'` claim rows exist in the table from before the repeatable gates and are invisible to them, so an account holding one is paid on its first repeat. Every line grants only its own milestone: a DCSS or NetHack win never back-grants the pickup it implies (a pickup the milestone stream missed is an ingest bug that shows as a missing badge, never a second payout off the win line), and Brogue's two endings and A Dark Room's are alternatives that grant only themselves. The clubhouse @bartender sells drinks through `ChipService::buy_drink`: one transaction that debits chips with a floor guard (`UserChips::apply` with `ChipMove::DrinkPurchase`, only pours if the balance keeps the 100-chip floor; source_ref = drink name) and upserts the buyer's `user_drinks` buzz (`late-core/src/models/drinks.rs`: decay-at-read drunk points, capped at `MAX_DRUNK_POINTS` 4000, wearing off on wall clock alone whether the drinker is online or not; the rate is derived from `DRUNK_SOBER_UP_HOURS` (12), the one dial, giving 334/hour). The AI decides drink and price (100-1000; out-of-range or unaffordable prices are refused uncharged so the debit always matches the quoted line) via `GhostService`'s ungrounded, schema-enforced JSON bartender flow in `app/ai/ghost.rs`; drunk levels print a `(word)` label on chat author headers from level 1 (tipsy) up (no background tint), and from level 1 they also scramble what the patron types: `ChatService::send_message` runs `app/chat/slur.rs` over outgoing **public-room** bodies (never DMs or private rooms) as the last step before the insert, so the slurred text is what gets stored and IRC/search/replies all see one version. Word interiors are reordered but first and last letters never move, which is what keeps even "wasted" legible. See `late-ssh/src/app/chat/CONTEXT.md` "Drunk Text" for the dials and the protected-token list. **The round** is the bar's chip sink (`late-core/src/models/drink_round.rs`): telling @bartender one of the literal `ROUND_PHRASES` ("round for everyone") buys every human online but the buyer a drink at `ROUND_PRICE_PER_PATRON` (100) a head through `ChipMove::RoundPurchase`, floor-guarded and burned whole with no credit reason anywhere; it counts on Top Chips as a debit like every spend. The phrase gate is deliberate: no model decides a spend whose price is the size of the room (a phrase in a `?` sentence or inside backticks is a question, answered by the model, not a bill), and `chat/slur.rs` reads the same list so a drunk patron's order is never scrambled out from under them (drunk text is stored, not rendered). Only the buyer is poured into, `ROUND_DRINK_POINTS` on the spot in the same transaction. Everyone else gets a `drink_credits` row, cashed only by ordering from @bartender (`ChipService::cash_round_drink`, which spends the credit closest to expiring, picked `FOR UPDATE SKIP LOCKED`), 24h to claim, worth a flat `ROUND_DRINK_POINTS` (400) of buzz with no chips moved and no `lifetime_spent`. Credits stack up to `MAX_OPEN_CREDITS` (3): a patron who was heads-down through three rounds is owed all three, a round past the cap skips them (and is free to the buyer for them), and the cap is the mechanic's only throttle. The pour reports who bought the credit it spent and what is left behind it, both of which @bartender appends to his line in code, never through the model. `ChipService::buy_round` charges for the credits the grant's own `RETURNING` produced, in the same transaction, so what was charged and what was poured cannot disagree; presence is `state::online_human_ids_excluding` over the in-process `active_users` map, single-replica on purpose. See `late-ssh/src/app/chat/CONTEXT.md` §9d The Round. Chat gilds are the one chip mechanic that lives entirely in chat rather than in the Shop: `g` on a selected message in a public room buys a permanent marker at one of three tiers, paying the author two thirds (`ChipMove::GildReceived`; both gild reasons count on Top Chips, a gild being paid for a message other people rated) and burning the rest, through `ChatService::gild_message` and one transaction over `chat_message_gilds` plus `UserChips::transfer_gild`. See `late-ssh/src/app/chat/CONTEXT.md` §9b Gilds.
- `PotService` (in `app/pot/svc.rs`) owns the weekly pot: a parimutuel raffle over `pots` / `pot_tickets` (`late-core/src/models/pot.rs`), tickets at 100 chips capped at 10 per player per UTC day (70 a week), drawn Monday 21:00 UTC. `ChipMove::PotTicket` is a floor-guarded debit (one ledger row per buy, not per ticket) and `ChipMove::PotWon` credits the drawn holder 80% of what the tickets paid in; the other fifth has no credit reason anywhere, which is the burn. Both are `counts_as_earnings = true`, like every spend and prize outside the tables and gifts. Every replica runs the 60-second sweeper; the draw is settled by a status transition (`UPDATE pots SET status = 'drawn' ... WHERE id = $1 AND status = 'open' RETURNING *`) inside a transaction that also credits the winner and opens the successor pot, all under the `pg_advisory_xact_lock` in `Pot::lock_open`, so exactly one replica pays and there is always exactly one open pot. A pot nobody entered rolls: no payout, no #lounge line. The per-user cap is checked inside the insert's `WHERE` and made exact by the buy holding the pot row `FOR UPDATE`. Distribution is the crown's shape: a process-shared `watch<Arc<PotSnapshot>>` (holdings kept private behind `tickets_for`, projected into `App.pot_view` on the ~1s tick, so `/pot` costs no query) plus a `pot_changed` LISTEN/NOTIFY task whose `PotChange::Drawn` payload raises the winner's banner on whichever replica they are on. Commands `/pot` and `/pot buy N`; the draw posts an `ActivityKind::PotDrawn` ticker line and a #lounge headline; there are no mid-week size lines and no sidebar pot panel: the size and countdown ride the status HUD, and `/pot` answers in full. Full contract: `late-ssh/src/app/pot/CONTEXT.md`.
- `AppFlagService` (in `app/flags/svc.rs`) serves the process-wide switches in `app_flags` (model `late-core/src/models/app_flag.rs`, a closed `AppFlag` enum mapped exhaustively to row keys, and a missing row is a load error, never a default). The crown's shape: one `LISTEN` on `app_flag_changed` (a trigger fires it on any insert or update) re-reads the whole table into a process-shared `watch<Option<AppFlags>>`; sessions read the `watch` on the tick path, and `None` (nothing loaded yet) reads as every switch off. Today it carries first contact's kill switch and fuse (`/haunt on|off`, `/haunt live on|off`) and the daily paper's kill switch and Outside-page switch (`/paper on|off`, `/paper outside on|off`); and the Artboard gallery's kill switch (`/mod artboard gallery on|off`); any new process-wide switch is a variant, a seed row, and a field here, never an in-memory atomic (multi-replica rule, §0).
- `CrownService` (in `app/crown/svc.rs`) owns the crown: one row in `crown_reigns` (`late-core/src/models/crown.rs`) held by one user, taken off them for `max(500, ceil(paid x 1.5))`. `ChipMove::CrownTaken` is a floor-guarded debit with no matching credit reason anywhere, so the whole price leaves the money supply; the ladder therefore ratchets with the buyers and is never tuned here. Like `ShopPurchase` it is `counts_as_earnings = false`: a debit never touches the Top Chips board. The take transaction takes a `pg_advisory_xact_lock` before the `SELECT ... FOR UPDATE`, because a take from a vacant crown has no row to lock and two racing first takes would otherwise collide on the `crown_reigns_single_open` partial unique index instead of the second paying the next rung. Distribution is the `StreamService` shape: a process-shared `watch<Option<CrownHolder>>` seeded by a `crown_changed` LISTEN/NOTIFY task (`start_listener_task`, wired in `main.rs`, reconnecting and re-seeding after 5s) plus a `broadcast<CrownEvent>` for the command answers; the notify payload (`CrownChange`) is what raises the deposed holder's banner on whichever replica they are on. A reign carries the first of the UTC month it was taken in and counts only while that month does, so the slot empties at the rollover with no sweeper and the month's last holder gets a `crown` profile award (`CRWN`, rankless) from the existing monthly snapshot. The glyph is folded into `App.name_flair` on the same once-a-second edge as the flair directory, so no render queries for it. Commands `/crown` and `/crown take`; every takeover posts an `ActivityKind::CrownTaken` ticker line naming both players plus a headline, a real un-prefixed `system` chat message in #lounge quoting the price and the next rung and naming both players as `@mentions` so the deposed holder gets a notification (`activity/filter.rs::lounge_headline`, the exhaustive twin of `lounge_includes`). Full contract: `late-ssh/src/app/chat/CONTEXT.md` §9c.
- `ArticleService` (in `app/chat/news/svc.rs`) pays one of the two chip rewards that live outside the Shop and the games (the jukebox pays the other, below): publishing an article credits `NEWS_SHARE_REWARD_CHIPS` (500) as `ChipMove::NewsShared`, minted rather than moved and counted as earnings. Both share surfaces reach it through `do_process_url`, and the credit itself lives in `Article::create_shared` so neither can pay a different amount. `source_ref` is the shared URL rather than an article id: the ledger row is what caps the reward at one per URL per user, and at `NEWS_SHARE_MAX_PAID_PER_DAY` (3) paid shares per UTC day, and it has to outlive a deleted story (`(user_id, source_ref)` is indexed for `news_shared`).
- `AudioService` (in `app/audio/svc.rs`) pays the other: queueing a YouTube track credits `SONG_QUEUE_REWARD_CHIPS` (100) as `ChipMove::SongQueued`, minted rather than moved and counted as earnings. Every submit surface (the Music Booth, `/audio`, a trusted submit, a re-queue from History) reaches it through `MediaQueueItem::insert_youtube`, which is the only path a submission may take, so none of them can pay a different amount or forget to pay; the credit is in the same advisory-locked transaction as the insert. **The track is never a gate**, unlike the News share it is otherwise built like: a repeat, a re-queue from History, a song somebody else put on an hour ago all pay, and the only question asked is how many this person has already been paid for today. `SONG_QUEUE_MAX_PAID_PER_DAY` (5) per UTC day is the whole of the gating, which is what keeps "why did that one not pay?" a single number the guide can state. `MAX_SUBMISSIONS_PER_WINDOW` (10 per 5 minutes) is a rate limit for the room's sake and would be a chip printer on its own. `source_ref` is the video id as provenance only; nothing reads it back. The closed `SongQueueReward` drives the metric and rides `SubmitQueueResponse.reward_chips` into the booth banner, so a track that came in past the cap never advertises chips. `late-ssh/src/app/audio/CONTEXT.md` §4 Submission reward.
- `BonsaiService` (in `app/bonsai/svc.rs`) owns bonsai persistence and activity: it plants the tree at first login, saves the branch graph, and pays the 200 daily chips behind the `bonsai_trees.last_watered` gate (`Tree::water_day`), once per UTC day for everyone. See `late-ssh/src/app/bonsai/CONTEXT.md`.
- `Activity` (in `app/activity`) owns the structured global user-action event type, channel helpers, and `ActivityPublisher` username lookup helper. `ActivityEvent` carries a dedupe id, `user_id`, `username`, display `action`, structured `ActivityKind`, category, and timestamp. The public surface is the #lounge system feed: `activity/lounge.rs` spawns one process-global task (started in `main.rs`) that drains the broadcast, filters through `filter::lounge_includes` (the ONE exhaustive kind+game match deciding what ships to #lounge: joins, table-game sits, door-game starts/boss falls/events, human-vs-human match wins, finished daily correspondence matches (one `DailyResult` line per match, win/loss or draw), and daily-puzzle solves (Sudoku/Nonogram/Minesweeper/Solitaire/Le Word/Rubik's Cube/Sliding Puzzle — their `GameWon` fires only in daily mode, so a once-per-board finish, not a practice grind) — never per-hand gambling wins, score-run arcade wins (Lateris/2048/Snake ride the hidden `GameScored` signal), per-mob kills, bonsai waterings, or bonsai losses (the bonsai is a private ritual; both its watering and its death are excluded)), throttles per-(user, event-shape) repeats inside 30 minutes, and posts each survivor as a persisted #lounge message authored by the lazily-ensured `system` bot user (`settings.bot` + `settings.system`, fingerprint `system-fp-000`) with the `· ` body prefix. The TUI never renders these as chat rows: `ChatState` diverts them at every ingestion point into a newest-first 10-item queue rendered as the one-row activity ticker in the composer-gap row of both Home chat surfaces — events pack left to right with compact stamps until the row is full (see `late-ssh/src/app/chat/CONTEXT.md`). Bodies never contain `@` so no mentions fire, and `ChatRoom::list_for_user_with_state` excludes `settings.system` authors so system lines never light unread badges; IRC sees them as normal PRIVMSGs from `system`. Per-session TUIs keep a broadcast subscription only to edge-detect friend joins for the friend-online banner (`tick.rs`); there is no per-session activity buffer, no seeded history, and no sidebar feed panel. Quest-only progress signals such as score submissions and settled hand counts use `ActivityCategory::Quest`, which is hidden from every public surface but consumed by `QuestService`. Structured kinds beyond `GameWon`: `GameStarted` (door-game entry), `BossSlain` (Lateania: only the three named realm crowns recognized by `boss_achievement_for`, gated on `KillOutcome.achievement` — the ~9 regional zone bosses stay dashboard-only), `SatDown` (published at the single `RoomGameRegistry::start_dashboard_room_join_feed_task` choke point where every room game's `SeatJoined` converges), and `DailyResult` (a finished daily correspondence match — one line per match, published from `DailyService::finish_events` via `ActivityPublisher::daily_result_task`; a win reads "won a game of Chess" — attributed to the winner, the loser never named — while a draw reads "drew with X at Connect Four", naming both since a draw shames no one).
...
- **Adaptive world tick** — each render pass returns `App::wake_hint() -> Duration` (read under the app lock, after the draw) and the loop sleeps exactly that long unless input or a `RenderSignal` wake lands first. Four tiers (`app/tick.rs` consts): `HOT_TICK` 66ms for full-rate surfaces (splash, post-input 2s window, active ultimate effect, house tables, an open arcade game), `ANIM_HALF_TICK` 132ms for the Clubhouse, any visible right sidebar (the eq strip + bonsai sway), the bonsai care modal and a profile hero showing a tree (the same sway), and the drawn pet box (Zen tile), `ANIM_QUARTER_TICK` 264ms for the aquarium surfaces (Zen tile + profile reef), `IDLE_TICK` 500ms floor otherwise. Frame edges are divisors of the one wall clock: the ambient music equalizer (`viz::render_eq`, a stateless synthesized spectrum driven by `marquee_tick`, no audio data), pet, bonsai sway, and clubhouse ambience paint on the `anim_half` /2 edge (~7.5fps); aquarium sim steps on the `anim_quarter` /4 edge (~3.8fps). A sidebar-visible session therefore never settles fully clean, but what holds the edge depends on the panels: the bonsai sway always animates, while the eq only animates when a client is paired and unmuted (`EqState`, audio CONTEXT §10). An unpaired session still repaints, and the strip it repaints is identical, so those frames diff to nothing and ship no bytes. The pet's clocks are wall-synced (`PetState::tick(wall_tick)`), so its wake matches its paint edge; the bonsai care modal rides the half tier because its only motion is the sway. Ticks at the floor only drain channels; an unprompted event (a chat message while idle) waits at most one floor interval. Advancing the world = `app.tick()`, render if dirty, ship the frame.
...
A blanket `changed = true` or fixed cadence needs a written justification at its call site (current survivors, all in tick.rs: splash typing, lobby modal 1Hz occupancy, DailyMatch 1Hz deadline clock, a running `/status` countdown's 1Hz HUD badge, the `anim_half` ~7.5fps edge shared by the pet box, bonsai sway, and clubhouse ambience, and the `anim_quarter` ~3.8fps edge for aquarium steps). The program summary, design rules, test gotchas, and open follow-ups live in SCALE.md (Render-Cost Program).
...
The input path sets `dirty` immediately after enqueueing bytes for the render task, without taking the app mutex. The render task clears `dirty` immediately before draining that queue under the mutex. Invariant: input that lands during a render flips `dirty` back to `true`, so the current frame may miss it, but the next loop iteration must pick it up.
...
Music binaries live in Cloudflare R2 (bucket configured via the `MUSIC_BUCKET` GitHub var), pushed to the Liquidsoap PVC at `/music/` by `sync_music.yml`, a manual-dispatch-only workflow (it wipes and re-copies every genre directory, so it is deliberately decoupled from terraform applies: run it when the bucket contents change, never as a side effect of shipping manifests). Playlists are `.m3u` files in `infra/liquidsoap/` in Liquidsoap `annotate:` format and live in git: git is the source of truth for playlists, licenses, and source URLs, never for binaries. ConfigMap changes (playlists, radio.liq, icecast.xml) trigger automatic rollouts via `config_hash` annotations on deployment templates, so no explicit restart job is needed.

#### Download tooling

- `scripts/fetch_cc_music.py` downloads from Bandcamp (via yt-dlp) and Internet Archive (via urllib) and generates `.m3u` playlists with ffprobe metadata. Supports `--genre` and `--m3u-only` flags. Ambient uses a curated FMA manifest inside the script.
- FMA CDN scrape pattern: FMA pages embed `fileUrl` in HTML as `https://files.freemusicarchive.org/storage-freemusicarchive-org/tracks/{hash}.mp3`. These are direct-downloadable without authentication; extract with a regex on the page source.
...
- Native SSH is the default launcher path. `--ssh-mode old` is the OpenSSH-through-PTY compatibility path, and `--ssh-mode openssh` is the OpenSSH-managed path for hardware-backed keys.
...
- The gallery (`app/artboard/gallery`): in view mode a rail on the left of the page (the page lands on it; Esc from the board goes back to it) lists the live board, the gallery listings (Ranking, Latest, Hall of fame, Mine), `Hang a piece`, and the archive browser. Hanging crops a frame of the live board into an immutable `artboard_pieces` row: at least 40 glyphs, at most 100x40, at least 75% painted by the hanger per cell provenance, three a day, no exact repeat in a month (content hash). Applause (`v`) is one per person per piece, free, revocable, never on your own, and closed once the piece's month is over. The hanger takes their own piece down with `x`, `x` (this month only); a take-down is a soft delete (`removed_at`): the row still counts against the daily cap and the per-month repeat rule, and leaves every listing. Month end: the `artboard` profile award ranks hangers by their best piece's applause (3 to count, earliest hang breaks a tie, `ROW_NUMBER` so a tie never pays twice) and pays 40,000 / 15,000 / 10,000 chips once, in the award snapshot's transaction; the month is settled by the first pass after the rollover and later passes pay nothing. The switch also blanks the splash podium and the paper's wall column. Last month's podium (`ART1`-`ART3`, read off the award rows) hangs on the splash, one place per login until the account has seen them all; yesterday's most applauded pieces, up to three, print in colour in The Late Edition. Switch `artboard_gallery_enabled` (`/mod artboard gallery on|off`); `/mod artboard remove <id-prefix>` takes a piece down, any owner, any month, the same soft delete. The web `/gallery` does not list pieces yet.
...
│   │       ├── bonsai/         # The bonsai branch graph: state, renderers, care modal, service
...
│   │       ├── lobby/          # Ctrl+G Lobby modal; daily/ + house/ game domains; see app/lobby/CONTEXT.md
│   │       ├── profile/        # Username/profile settings and stats
│   │       └── workspace/      # Backtick workspace cycle across daily/house/arcade/door games; see app/workspace/CONTEXT.md
...
- **Why the modal keeps them out of its draft:** `SettingsModalState::draft` is
  ...
  would republish one device's layout as the account default on any unrelated
  settings edit, which every unconfigured key would then inherit; pinned by
...
- **`auto`:** resolved against the live terminal width every frame
  ...
  both.
- **Bool mirrors:** `users.settings.show_right_sidebar` /
  `show_room_list_sidebar` bools are written alongside their `*_mode`
  strings, so a rollback to a binary that reads only the bools keeps working.
...
| Tree (bonsai) | `bonsai_trees` | `user_id` UNIQUE, seed, last_watered DATE (the daily chip gate), is_alive, vigor, water_stress, last_simulated_date, branch_graph JSONB, selected_branch_id, mode, badge_glyph, planted_at, state_revision (stale-write guard). Created as `bonsai_v2_trees` (056), renamed and emptied by 177; the classic `bonsai_trees`, `bonsai_graveyard`, and `bonsai_daily_care` tables were dropped there. |
| GamePayoutClaim | `game_payout_claims` | `UNIQUE(user_id, game, payout_kind, period_kind, period_key)`, reusable chip-payout claim rows; Asterion escape uses `period_kind=utc_day`, Tron wins use `period_kind=cooldown` for DB-backed per-player reward cooldowns, and the repeatable door milestones use `GamePayout::grant_multi` to write an `event` row (run/character identity) and a `cooldown` row (the 7-day lockout) in the same transaction as the credit, all-or-nothing. Every row of a multi-key grant carries the full amount (the table's CHECK forbids a zero), so `chip_ledger` — one row per credited grant — is the only honest total |
| PetCompanion | `pet_companions` | `user_id` UNIQUE, `species` (`cat`/`dog`/`bird`, CHECK-constrained, `PetSpecies`), `name`, `adopted_at`, `mood` (`PetMood`: purring/proud/sulking/chatty/asleep/vibing/idle, CHECK-constrained) and `mood_since`. Migration 181 dropped every care column: the pet is never fed. The mood is inferred by the owner's session (`late-ssh/src/app/pet/state.rs`), written by `PetCompanion::set_mood` on each change and set to `asleep` when the session ends, so a profile can show it. `ChipMove::PetFed` survives only as ledger history. |
...
| Pot | `pots` | The weekly raffle (migration 160). `status` IN (`open`, `drawn`, `rolled`) with a partial unique index on `(status = 'open')` so at most one is open ever; `draws_at` is the next Monday 21:00 UTC, `ticket_price` 100. `winner_user_id`/`ticket_count`/`payout_chips`/`drawn_at` are all NULL while open and stamped once at the draw, enforced by one CHECK per status (a drawn pot is told from a rolled one by `ticket_count > 0`, not by the winner, whose account can be deleted out from under a settled row). |
| PotTicket | `pot_tickets` | One row per buy, never updated: a player's holding is the sum of their rows, and `chip_ledger` has one `pot_ticket` debit per row. A live pot's size is `SUM(count) * ticket_price`; there is no stored running total and no house wallet. The 10-per-player-per-UTC-day cap is checked inside the insert's `WHERE` and made exact by the buyer holding the pot row `FOR UPDATE`. |
...
| ArtboardPiece / ArtboardPieceVote | `artboard_pieces`, `artboard_piece_votes` | Gallery pieces (migration 174): `user_id` FK, `title`, `width`/`height`, cropped `canvas`/`provenance` JSONB, `glyph_count`, `own_share_percent`, `content_hash` UNIQUE with `period_month`; applause rows `(piece_id, user_id)` PK with `author_user_id` denormalized for the no-self-applause CHECK. Model `late-core/src/models/artboard_piece.rs`. |

**Key enums:**
- `Screen`: `Dashboard`, `Arcade`, `Games`, `Artboard`, `Lateania`, `Rebels`, `Nethack`, `Dopewars`, `Bashquest`, `Codekeep`, `GreenDragon`, `Profiles`, `Clubhouse`, `Nightcap`, `DailyMatch`, `HouseTable` (the door-game screens — Lateania/Rebels/Nethack/Dopewars/Bashquest/Codekeep/GreenDragon — are reached only through the Games hub, and `DailyMatch`/`HouseTable` only from the Lobby modal; none are in the tab cycle. `Profiles` is the people list (one row per user with projects or a work card) on key `5` (rendered by the `directory` slice); `Clubhouse` is the tavern on key `0`, first in the Tab cycle and the landing screen for every session — see `late-ssh/src/app/clubhouse/`. `Nightcap` is the small bar reached with `n` from the Clubhouse, Esc back; also absent from the tab cycle — see `late-ssh/src/app/nightcap/`). `Dashboard` is rendered as Home and owns the chat room rail/center. News, Mentions, RSS, Voice, and Discover are synthetic room-like entries within Home chat. Showcase/Projects and Work data still use chat-adjacent services and unread cursors, but their UI lives on the Profiles page 5, not the Home rail or room jump picker.
...
- **Deploy skew rule.** A profile change that adds or removes a required secret must ship its image before or together with the terraform apply that changes the injected env; an old image against a new pod template crash-loops on `... must be set` (see the 2026-07-31 incident in the Runbook). Since every release is image-only (`deploy_service.yml` never runs terraform), the ordering is explicit: deploy the tolerant image first (`vX.Y.Z` for ssh, `-web` for web), then ship the pod-template change with an `-infra` release. A single `-full` release does both legs in that order for ssh, which is the one-tag way to ship code and its manifest change together; it does not double-roll ssh, because terraform holds `ignore_changes` on the ssh container image and injects every secret by `secret_key_ref` rather than by value, so the apply re-rolls ssh only when the diff genuinely edits its pod template. A change touching `late-web/src/config.rs` needs a `-web` release; an ssh release never applies infra drift as a side effect.
...
- **VictoriaLogs `stats` panels are numbers-only:** in a `queryType: stats` target the `by (...)` fields become labels and every other returned column is parsed as a float, so a string column fails the whole panel with `unable to parse log value to float64`. Never select a name with an aggregate (`max(username) runner`); group by it instead (`stats by (user_id, username)`). To show a field in a `logs` panel, format it into the message (`| format "[<username>] <_msg>" as _msg`): that panel renders `_msg` and nothing else.
...
The SSH app assumes a single process. These in-memory structures would need to be externalized (Redis / Postgres) for multiple replicas:
...
| Nightcap `SharedSeats` | `state.rs` (`State.nightcap_lobby`) | One process-global seat map for the small bar out back | Same exception as the Clubhouse's `SharedLobby`, same fix if ever needed |
...
- `#announcements` is read-joinable like other permanent public rooms, but only admins may post there; enforce this in the chat service send path, not only in the UI. There is no login announcements modal any more (removed 2026-09-11): yesterday's `#announcements` posts print verbatim at the top of The Late Edition (`late-ssh/src/app/paper/CONTEXT.md`), and the room's unread count behaves like any other room's.
...
- **SSH send failure is terminal for render task:** if `handle.data` returns `Err` (closed/broken channel), `render_once` returns an error so the render loop stops and closes the channel once, rather than logging warnings every 66ms forever
...
- **Game services publish Activity wins:** Arcade daily services and door games publish structured `ActivityEvent::game_won(...)` callouts; house tables publish sit-downs only (`ActivityKind::SatDown`, by decision). Arcade details live in `late-ssh/src/app/arcade/CONTEXT.md`.
- **Bonsai is one tree for everyone (2026-09-08):** the living branch graph that shipped as the "Dynamic Bonsai" shop unlock is the only bonsai; the classic stage ladder, its daily care rows, the graveyard, the shop item, and the `bonsai_variant` slot are gone, and migration 177 reset every tree to a fresh seed rather than migrating growth points. `BonsaiService::ensure_tree` plants the root at first login. Growth comes from watering and the elapsed-day catch-up at login only; there is no in-session tick, so a death (stress maxed and vigor at zero) is detected at the next login. The global `w` opens the care modal; watering happens inside it. Rules, keys, and renderers: `late-ssh/src/app/bonsai/CONTEXT.md`.
- **Chat username badge order:** Chat author labels render top-3 last-completed-UTC-month leaderboard award badges first as one bracketed group, then special allowlist badges (`mod`, `developer`, `artist` order), the bonsai glyph, rented chat badge, rented flag, and finally the `/brb` moon when any active session for that user is away. The bonsai glyph is the precomputed `bonsai_trees.badge_glyph` (ladder `·` `⚘` `🌱` `🌲` `🌳` `🌸` `🌼`, scored from live branch presence times health); a dead tree, or an account that has not logged in since the reset, shows none.
- **Bonsai care modal:** `w` waters or replants, `tab`/`n`/`shift-tab`/wheel select a live tip, `hjkl`/arrows wire it, `x` cuts it and everything above, `p` pinches toward a leaf pad, `s` marks a split, `c` copies the share snippet, `?` opens the Bonsai help section. The modal is the pot: the tree lives in a fixed 81x26 canvas drawn 1:1 there, capped at 128 branches ("full: cut to make room").
- **Pet Companion is a Hub Shop unlock:** `pet_companions` stores the species (cat, dog, or bird: `t` in the Shop rings through them), the name, and the last inferred mood. Visibility/access is gated by Hub Shop entitlements from `user_purchases`. The marketplace SKU is `pet_companion` (`PET_COMPANION_SKU`). The pet lives on the Zen page only (a tile, `late-ssh/src/app/zen/CONTEXT.md`) and on the profile as a portrait in its mood. It has no care and no command: `PetState` reads the session every tick (`mood_at`, precedence: purring after a click on it for 2 min, proud 30 min after any own win, sulking 10 min after a loss, chatty 10 min after a sent message, asleep after 10 quiet minutes, vibing while a paired client plays unmuted, else idle) and walks after the terminal cursor while it is inside the tile (`App::last_mouse`, `Perch`); a sulking or sleeping pet does not come. Wins and deaths arrive on the activity feed (`GameWon`, `GameScored`, `BossSlain`, `DailyResult`, `GameLost`), a lost daily match through `DailyTick::own_loss`, a sent message through `ChatState::last_own_send_at`.
- **Bonsai sway is stateless:** the modal, the sidebar's fixed 21x13 preview, and the profile hero all shift the same rendered lines by a sine of the wall tick (`bonsai::render::apply_sway`), on the `anim_half` edge.
...
- **Terminal side-channel commands bypass the frame diff:** OSC 777 (kitty/Ghostty/rxvt-unicode/foot/wezterm/konsole/mlterm), OSC 9 (iTerm2), OSC 52 clipboard copies, and Kitty/iTerm2/Sixel image previews are written to `App::pending_terminal_commands`, not into the ratatui frame. `late-ssh::ssh::render_once` drains that buffer **after** pushing the frame diff and sends each payload as a separate `handle.data` call. Writing them inline with `write!(self.shared, …)` would slip them into the diff and get re-emitted on every redraw. The session emits XTVERSION (`CSI > q`), iTerm2 feature-reporting (`OSC 1337;Capabilities`), and DA1 (`CSI c`, sent last as a sync point since replies arrive in probe order) probes alongside the other alt-screen setup bytes. The input parser consumes XTVERSION DCS replies, `OSC 1337;Capabilities=...` replies, and DA1 `CSI ? ... c` replies to enable raster image previews. Kitty-family detection includes Kitty, Ghostty, WezTerm, Rio, Warp, and Konsole; iTerm2-family detection includes iTerm2, mintty, hterm-style identities, and `TERM_FEATURES`/Capabilities reports advertising `FILE`; Sixel detection includes Windows Terminal/foot/contour/mlterm/sixel identities plus `WT_SESSION`/`WT_PROFILE_ID` forwarded by native `late.exe`, plus DA1 attribute 4 as a fill-in-only signal: `apply_primary_device_attributes` never displaces an already-detected protocol, so Kitty/iTerm2 strictly win over Sixel when both are advertised (e.g. WezTerm). Chat rows always render the RGB block image fallback; native Kitty/iTerm2/Sixel image data is fetched lazily and emitted by the explicit selected-message image modal plus the small set of non-modal producers that opt in (currently the Sliding Puzzle board — see `late-ssh/src/app/arcade/CONTEXT.md`; the chess board art does not, and its rasters can linger on protocols without delete-by-id). Sixel payloads are only generated for Sixel sessions; the image modal reports its image cell capacity back through `TerminalImageFrame` so Sixel fetches encode to a size that fits (first fetch deferred one frame after modal open), and payloads over the hard byte cap fall back to the RGB block preview. If the PTY `TERM` is tmux, full image previews are intentionally disabled and chat uses the RGB block fallback; no tmux graphics passthrough is attempted. Direct terminals still get Kitty cleanup commands on enter/leave-alt-screen. Kitty images use late.sh-owned ids in the `0x4C000000..0x4CFFFFFF` range plus a dedicated z-index for cleanup. Splash input handling avoids treating terminal replies as user `Esc`. **Persistent-raster cleanup is pre-frame:** neither iTerm2 nor Sixel has Kitty's delete-by-id lifecycle, and on some terminals (notably WezTerm, which is iTerm2-family here) the raster layer persists above cell content until cells are written to. `TerminalImageRenderState::pre_frame_persistent_raster_wipe_bytes` is called in `App::render` **before** `terminal.draw`, writes wipe spaces (`\x1b[0m` + cursor-position + spaces per row) directly to `self.shared` so they land in the output buffer ahead of ratatui's frame diff, and only fires on transitions: image modal closed, image swapped, foreground overlay opened, the screen changed under a non-modal placement, the non-modal raster's own identity changed (`persistent_raster_tag`, which folds its rect together with its content), or the detected protocol changed. Re-emission is also suppressed by `build_commands` while a foreground overlay (`show_settings`, `icon_picker_open`, the stream and chat-history modals, etc.) is open so the image doesn't repaint over modal cells. **Opaque same-coverage replacement:** when consecutive frames place the same number of fully opaque images over exactly the same cells, `build_commands` emits only the changed ones instead of a full cleanup-and-retransmit. **Kitty transmits pixels once per cleanup epoch:** the image id is a fold of the pixel content (`cache_key`) and the placement id a fold of the message id, and `TerminalImageRenderState::kitty_transmitted` records what the terminal already holds, so a cell whose image was transmitted earlier gets an `a=p` placement command of a few dozen bytes and the replacement path deletes only the old *placement* (`a=d,d=i,i=,p=`), never the image data. A Sliding Puzzle move is therefore two placements and two placement deletes with no PNG on the wire. The record is emptied whenever `kitty_cleanup_commands` goes out, since those free every id in the late.sh range. **A dropped terminal command resets the raster state:** `render_once` treats a timed-out side-channel write (most often one chunk of an image transmission) like a dropped frame, calls `force_full_repaint` (which resets `TerminalImageRenderState`), and abandons the rest of that batch, so the next frame re-emits every image instead of leaving a half-transmitted tile that the placement diff believes is on screen. The output-budget guard keeps a persistently busy link from turning that retry into a flood.
- **Notification domain lives in `late-ssh/src/app/notify`:** one module owns the typed `Kind` enum (`Friends`/`Dms`/`Mentions`/`GameEvents`/`Streams`, `key()` matches `users.settings.notify_kinds`), all `Notification` copy constructors, and the OSC 777/OSC 9 byte building. Producers anywhere push through a cloned `notify::Notifier` (ChatState holds one for friend joins/DMs/mentions/new polls, plus the friend-went-live alert, which is the one friend-shaped notification on `Streams` rather than the always-on `Friends`, so a nightly streamer cannot force you to give up their login pings; `App::tick_stream` pushes the "someone opened your stream" alert off `StreamEvent::ViewerJoined` on the same `Streams` kind; `DailyState` pushes daily-match "your turn" edges; `HouseState::notify_turn_edges` pushes house poker/blackjack "your turn" edges off `HouseTableRegistry::awaiting_action`; `tick.rs` pushes the `/status` countdown completion off `App::status`, reusing `GameEvents` rather than adding a fifth kind and settings row for one timer). `render.rs` drains `App::notify_outbox` once per frame; the outbox picks the first pending whose kind is enabled (`Friends` always passes — `/friend` is the opt-in), honors the shared `notify_cooldown_mins`, and drops the rest rather than queueing. Adding a new kind means: (1) add a `Kind` variant + copy constructor in `notify/mod.rs`, (2) add a matching toggle row in the settings modal UI/state, (3) push it from the relevant producer. No tmux DCS wrapping — tmux is explicitly unsupported.
- **Profile notifications default to all-off:** a new account has `notify_kinds = []` and `notify_cooldown_mins = 0` in `users.settings`. The notify outbox only fires if the kind string is present in the user's array, so a brand-new account is silent until they opt in through the settings modal. `notify_kinds` is the whole model; there is no focus-tracking policy.
- **`Profile` is a view, not a table:** there is no `profiles` table; username, notify settings, and theme live on `users` (column + `settings` JSONB). `late_core::models::profile::Profile` is a projection loaded via `Profile::load(client, user_id)` and saved via `Profile::update(client, user_id, params)`, which merges into `settings` with `settings || jsonb_build_object(...)` to preserve unrelated keys (theme_id, ignored_user_ids) under concurrent writes. Profile also exposes JSON-backed system fields (`ide`, `terminal`, `os`) plus language tags (`langs`, normalized to up to eight `#tag` values) and `users.created` as `created_at`; the read-only profile modal loads profile + chip balance via `Profile::load_with_chip_balance()` (layout in `late-ssh/src/app/profile_modal/CONTEXT.md`).
...
# There are no seed scripts; local data is created through migrations and app
# flows. Chips for a local account come from the admin-only /grant chat command.
...
8. Music missing from PVC → Dispatch the `sync_music` workflow for the environment (syncs from R2; manual only, it no longer rides an infra deploy). For manual recovery: `aws s3 sync s3://$MUSIC_BUCKET/ ./music/ --endpoint-url $S3_ENDPOINT` then `kubectl cp` each genre dir individually into the pod.
...
#### 2026-09-03: SSH connection flood took the ingress down (edge outage ~5 min, all sessions dropped)

- **Symptom:** from 06:59Z a TCP connection flood hit port 22. At ~07:02:40Z the ingress-nginx controller failed its liveness probe (1 s timeout on `/healthz`), kubelet killed it, nginx drained for four minutes (listeners closed, every public port 22/80/443/6697 refused), restarted at ~07:07Z and dropped all 18 proxied SSH sessions. `service-ssh` itself never restarted. Locally the CLI reported "Network is unreachable" because the machine has no IPv6 route and the IPv4 attempt was the refused one; the CLI surfaces only the last error.
- **Evidence:** VictoriaLogs shows late-ssh going from ~10 lines/min to 8.8k (06:59), 25.6k (07:00), 41.7k (07:01). Messages: 47k `failed to resolve proxy protocol header; dropping connection`, 29.6k `connection limit reached, rejecting new client` (the 1000-permit global semaphore was exhausted from 06:59 on, so any real login in that window was rejected at auth), 2.3k `ssh rate limit exceeded`, 2k `per-ip limit reached`. `late_ssh_connections_total` peaked at 16.3k/min (those are only connections that got past the PROXY header). Phase 1 (06:59-07:03) was distributed: 361 distinct client IPs in the old nginx log, ~25 connections each, and 9,190 of them were still open (>200 s) when nginx drained at 07:06, i.e. they were held, not churned. Phase 2 (07:08Z onward, still running at 07:14Z, ~6k/min) is a single IP, `189.217.206.172` (39.5k rate-limit rejects in 8 minutes). Ingress container CPU went 0.04 to 4.9 cores, node root cgroup 1.7 to 7.2 of 8 cores at 07:03, so the controller's Go healthz endpoint starved behind nginx workers and the 1 s probe tripped. Ingress memory 149 to 696 MiB, late-ssh working set 700 to 1814 MiB from held connections. Kubelet PLEG p99 stayed <50 ms: no node stall, pure CPU contention. No prior minute in the last 7 days exceeded 300 connections/min.
- **Verdict:** proven. A port-22 DDoS, first a ~360-host botnet holding connections open, then one host churning. The ingress died from CPU starvation of its health endpoint, not from nginx limits (worker_connections 16384 x 8, nofile 1M, none hit; no nginx error lines besides upstream resets).
- **Weaknesses it exposed:** (1) over-limit connections in `ssh.rs` `new_client_with_addrs` are not dropped: they get a full SSH handshake and only fail at auth/channel-open, so a bot that never authenticates holds its global permit, its tokio task and russh buffers until `ssh_idle_timeout` (3600 s) or keepalive reaps it; that is how 1000 permits vanished in one minute and real users got rejected. (2) The global semaphore is acquired before the per-IP rate limiter, so rate-limited connections still burn permits. (3) The ingress liveness probe has a 1 s timeout and the controller has no CPU limit or priority, so any traffic spike that saturates the node converts into a 300 s drain outage of every port. (4) There is no edge firewall in `infra/` (no `hcloud_firewall`, no nginx per-IP limits on the TCP services), so the only per-IP defence is inside late-ssh, after nginx has already paid for the connection.
- **Fixes shipped (2026-09-03, same day):** (a) `ssh.rs`: admission moved ahead of the handshake. `Server::admit` runs the per-IP rate limiter, then the per-IP count, then takes the global permit last; any refusal returns before `run_stream`, so the socket closes on the spot and holds nothing. `over_limit` and the auth/channel-open rejections are gone; a refused client sees a TCP close, not an auth failure. New metric `late_ssh_connections_rejected_total{reason=proxy_header|rate_limited|per_ip_limit|global_limit}`; `late_ssh_connections_total` still counts every accepted socket, so rejected/accepted is the flood ratio. Tests in `ssh_test.rs`: rate-limited socket closed without a banner, rate-limited peer does not consume a global permit (client bound to 127.0.0.2), global limit closes the socket and frees the slot on disconnect. (b) `infra/ssh-tcp.tf`: ingress liveness/readiness probes 5 s timeout x 6 failures (was 1 s x 5 / x 3), controller requests 250m/256Mi and a 3-core CPU limit so nginx cannot take the node. Not done on purpose: per-IP `limit_conn` in nginx stream and HAProxy stick tables (marginal once (a) closes sockets instantly; revisit if a flood ever makes HTTP slow), and any IP blocklist (Hetzner Cloud Firewall is allow-only, and phase 1 used 361 addresses).
- **If it repeats:** `late_ssh_connections_total` > 300/min is abnormal, and `late_ssh_connections_rejected_total` by `reason` says which gate is doing the work; `stats by (_msg) count()` in VictoriaLogs tells the phase (proxy-header timeouts = nginx overloaded, `connection limit reached` = permits exhausted, `rate limit exceeded` by `ip` = who). Old nginx log (`kubectl logs --previous`) gives client IPs and per-connection durations; a drain batch of >200 s connections means held sockets. Node root cgroup CPU near 8 cores plus ingress healthz timeouts is the kill signature.
...
- **Still standing on 2026-08-30**, unverified: `AI_MODEL` moved to `gemini-3.7-flash` and nobody re-ran the curl. `generate_json_with_search` still asks for JSON through the prompt only, so nothing depends on the answer today; re-test before ever putting `responseMimeType` back on a grounded call. The budget half of this outage (thinking tokens billed as output and spent out of `maxOutputTokens`, leaving a body of nothing but `usageMetadata`) is a property of the Gemini 3 Flash family, not of 3.6, and applies unchanged.
...
#### 2026-07-22: service-ssh OOM during Hacker News traffic surge
...
One global overlay owns general app help plus the Pair, terminal FAQ, and Hub Guide topics. `?` opens it globally when not composing, except Artboard keeps its local page help. The default/first tab is Pair, which steers to the CLI first and carries the static late.sh/listen link and QR for listening without it.
...
| `h` / `j` / `k` / `l` / arrows | Bonsai modal | Wire the selected tip: bias where it grows next |
| `x` | Bonsai modal | Cut the selected branch and everything above it |
| `p` | Bonsai modal | Pinch the selected tip; three pinches over separate waves set a leaf pad |
...
| `PageUp` / `PageDown` / `g` / `G` | Read-only profile modal | Page, top, bottom |
...
- **Ratatui pinned at 0.30.2 / ratatui-widgets 0.3.2 / ratatui-textarea 0.9.2.** `App` must be `Send` for the russh `Handler + Send` bound, which needs upstream `CellEffect` to be `fmt::Debug + Send + Sync + UnwindSafe + RefUnwindSafe` (it is at 0.30.2, so `Block`/`TextArea<'static>` are `Send`). Re-check on any bump.
...
- **Every top-level screen change ends in `force_full_repaint`.** `App::set_screen` (`app/state.rs`) calls it once after assigning `self.screen`; no screen makes its own call on the way out. Two layouts rarely disagree on every cell, and a wide glyph the new layout only half-overwrites survives ratatui's diff: leaving Home for the Arcade would strand the sidebar's bonsai emoji on screen. The cost is one full frame per page switch, which most transitions already paid. A same-screen `set_screen` call (pressing the nav key for the page already open) changes no layout and skips the clear.
- **SSH compression is disabled server-side.** `russh::server::Config.preferred.compression` is pinned to `[compression::NONE]` in `ssh.rs`; russh otherwise advertises zlib by default, and a client that negotiates it (`ssh -C`, `Compression yes`) drops mid-session with "closed by remote host" on the first keystroke that moves a character. A TUI frame diff is small and already poorly compressible, so there is nothing to win by re-enabling it.
- **Never call `Terminal::resize()` on the SSH backend either.** Since ratatui-core 0.1.2 (ratatui 0.30.2) its fixed-viewport clear queries `backend.size()`, which reads the controlling tty (crossterm opens `/dev/tty`, falls back to stdout — a pipe under the SSH daemon and test runner, so both fail). `App::resize` (`app/state.rs`) instead rebuilds the terminal with `Terminal::with_options` + `Viewport::Fixed` (pure state construction, no backend I/O — same call `App::new` makes) and calls `force_full_repaint` for the client clear + full redraw that `resize()` would otherwise perform.
