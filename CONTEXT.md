...
- **Verdict:** confirmed cgroup OOM; allocation source unproven. The final sub-second jump is compatible with one large per-session allocation, while the two earlier recoverable multi-GiB spikes suggest a repeated render/session/bootstrap amplifier under burst traffic. There is not enough evidence to attribute it to a hostile resize, paired-client payload, the username-conflict loop, or Postgres.
- **Fixes shipped:** none during the incident; Kubernetes self-recovered. Do not scale `service-ssh` horizontally as an emergency response because Clubhouse and other session state remain single-process.
- **If it repeats:** preserve the previous container immediately; correlate 5-second `container_memory_{working_set_bytes,rss}` and `late_ssh_sessions_active` around the kill; grep for resize/clipboard/pair markers from the OOM #2 entry; and inspect the final session/bootstrap, web-tunnel, and render events. Add allocation/RSS telemetry around `App` bootstrap and terminal buffer resize before the next traffic event so a sub-scrape spike can be tied to a session token and dimensions. Treat increasing the 4 GiB limit only as temporary headroom, not a root-cause fix.
...
| _Lateania / NetHack / DCSS / Usurper / Green Dragon / Rebels / dopewars_ | — | Active | Live door-game screens, not top-level tabs. Launched only from the Games hub (page 3); `Esc` (Lateania) or quitting the game (Rebels/NetHack/DCSS/Usurper/dopewars, e.g. `Ctrl-C`, `Q` at Usurper's menus, or `S` save in the roguelikes) returns to the hub. Per-game behavior lives in each door's CONTEXT.md (`lateania/`, `greendragon/`, `nethack/`, `dcss/`, `usurper/`, `dopewars/`). |
...
| arrows / `h`/`l`/`j`/`k` | Games hub (page 3) | Switch the selected door-game card (Lateania / NetHack / DCSS / Usurper / Green Dragon / Rebels / dopewars) |
