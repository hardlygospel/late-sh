...
- Last updated: 2026-09-18 (companions are never released: `pets::Kennel` keeps every owned pet, one per species; a tame never displaces the pet at your heel and only trains Taming when the species is owned; swap at a Stable; see "The kennel" in §7. Stairs on the map are `⇓`/`⇑`/`⇕`. The standing keys show `: waypoint` and `/ warp` instead of say/ranks/leave. Before that, 2026-09-17: player-feedback pass, ported from a sibling standalone deployment: the Shattered Archipelago's tier formula had its 20 islands landing *below* Kaelmyr's own 20 zones (`isle + 14` against Kaelmyr's `z + 32`), so a player could out-level every island just from a Kaelmyr grind - this directly contradicted "The lands" table in §7, which has always documented the Archipelago as Lv75-100, past Kaelmyr's Lv69-80; the bug is that the code never actually matched that claim. Fixed: `tier = isle + 52`. The Archipelago also gained its own 200-item generated catalog (`archipelago_items`/`archipelago_loot`, IDs `5000..5200`, `Band::Archipelago` unchanged) instead of reusing `reaches_loot`, so its mobs and bosses stop handing back Sundered Reaches repeats; boss find-item odds ramp up through the back half/quarter of the 20-island chain, and the last three islands are `apex` fights (+25% boss hp/xp, +15% damage). Added **Thornveil Falls** (rooms `34000+`, `extend_thornveil`, hung off Broceliande's own deepest chamber), a sixth continent and a new `Band::Thornveil` riding the exact same tuning row as `Band::Kaelmyr` - a parallel endgame track overlapping Kaelmyr's lower tiers, not a crown (not in `CROWNS`/`CROWN_TARGETS`, same as the Archipelago), so Kaelmyr isn't the only late-game road. **Not yet arena-verified** - see §9 and §11: the real level/dps numbers for both changes need `make arena` / the `arena_report` tuning loop before they can be trusted as measured rather than modelled. Before that, 2026-09-16: the room panel reshaped: a responsive side rail, room-specific actions promoted to the top, the standing-key catalogue cut to a few packed lines pointing at the `?` guide and anchored to the floor of the rail, and the panel finally scrolls with vitals pinned. See "Panels" in §5. Also: the shops scale with the character: each gear shop also stocks its own slots from the generated realm catalogs at `PlayerState::market_tier`, derived from level against `world::CROWNS`' kit rows less `MARKET_LAG_PCT` and capped by the gate titles, so gold is a floor under a straggler and a real late-game sink instead of dead weight after L37. Consumables stay off the market on purpose. See "Items, shops, and rewards" in §7. Before that, 2026-09-15: mounts removed; every companion on one ladder via `PetSource`/`PetSpecies::rung`, the Stable grown to eight pets reaching rung 35, and the tame-50 pile-up spread over taming 46-54 so no beast is dead content. See the Animal Taming section, `FeatureKind::Stable`, and §9.)
- Status: Active
...
All nineteen `REGIONS`, in the order the ladder below plots them. "Level" is the displayed level of the mobs homed there (`region_progress`); bosses are listed separately because in several lands they sit well above the trash around them. **Thornveil Falls' row below is an estimate from its tier formula against Kaelmyr's own measured numbers, not an arena reading** - it hasn't been through `make arena` yet (see §9/§11); treat it as modelled until someone runs the real yardstick.
...
| Thornveil Falls · 12 zones | 34000+ | ~Lv60-70 (estimated) | 12, ~Lv65-73 (estimated); deepest Thornveil, the Voice Beneath the Falls |
...
Thornveil Falls                                         #####                   12 bosses, ~Lv65-73 (est.); deepest Thornveil
the Archipelago (portal)                                    ###############     20 bosses, Lv75-100
```

The road is the right-hand half: the seven crowns to L35, the seals at L40, the Frontier to the King at L55, the Reaches to Yssgar at L65, Kaelmyr to the two Kaethyrs at L75 and L80. Everything to the left of the Frontier is ungated side country where a character levels between crowns; the Archipelago is the prestige farm past the last one - and it now actually sits there in practice, not just on paper: its tier formula used to land it *under* Kaelmyr, so this table's own Lv75-100 claim was aspirational rather than measured until the fix (see the Metadata entry and §9). Thornveil Falls sits beside Kaelmyr rather than past it - a second road through the same stretch, not a longer one.
...
- **The measured power ladder (read this before touching any tier).** `tier` is a generator input, not a power level, and **is not comparable across bands**: `tune_spawn_balance` applies a different hp/dmg/xp row per `Band`, so two lands at the same `tier` can be a factor of three apart. Always compare *displayed* levels (`MobSpawn::level`) and post-row damage. Measured on this branch:

  | land | trash lvl | boss lvl | deepest trash dmg | deepest boss dmg |
  |---|---|---|---|---|
  | Kaelmyr (`z + 32`) | 63-78 | 69-80 | 214 | 348 |
  | Thornveil (`z + 30`) | 58-69 | 64-68 | ~149 | ~254 |
  | Archipelago, `isle + 14` (pre-port) | 69-100 | 75-100 | 361 | 555 |
  | Archipelago, `isle + 52` (current) | 100 flat | 100 flat | 505 | 1187 |
  ...
  - **A boss cannot read L100 without out-hitting the final crown.** `level_for_bite` discounts a boss's bite by `BOSS_BITE_PCT` (85) and a regular's by `TRASH_BITE_PCT` (70), so trash reads L100 at 359 damage but a boss needs 436, which is past the Ascendant's 397. Trash at L100 under a L80-capped final boss is reachable; bosses at L100 are not.
  ...
  - **XP does not follow damage.** The Archipelago row pays `×3/2` xp on doubled hp, and its xp base (`210 + tier * 40`) is half Kaelmyr's (`420 + tier * 45`). Measured xp per point of mob health: Kaelmyr z19 **0.93** trash / **0.68** boss against the Archipelago's **0.39** / **0.22**. The Archipelago is currently the *slowest* endgame ground to level in, not the fastest. Gold is `xp/5` (`gold_for_kill`), so it follows. Fixing a tier does nothing for this; the xp coefficients and the band row are separate knobs.

- Keep the Archipelago's ID ranges aligned: 20 islands, rooms at `20000+` (`archipelago::ARCH_BASE`/`ARCH_STRIDE`), mob IDs at `970000..980000` (`ARCH_SPAWN_ID_START`, `Band::Archipelago`, its own dedicated row rather than sharing Kaelmyr's), generated gear at `5000..5200` (`ARCHIPELAGO_ITEM_BASE`/`ARCHIPELAGO_TIERS`, `power_offset = 60`), and regional finds at `3800..3840` (`ARCHIPELAGO_FIND_BASE`). **Island tier is `isle + 52`. OPEN: this is under review and should not be treated as settled** - it was derived by comparing tier literals across two bands with very different rows, and measured it puts every mob on all 20 islands at L100 while out-hitting the final crown by up to 3x (see the measured ladder above). The xp/gold half of the complaint it was meant to answer is real and is *not* addressed by the tier. The claim it replaced was that a lower tier is the bug this port fixed (Kaelmyr's own zones out-xp'd/out-golded every Archipelago island; gold is derived straight from xp via `gold_for_kill`, so an xp regression silently regresses gold too) and it is also what "The lands" table in §7 has always claimed the Archipelago's level actually is. `archipelago_boss_loot` weights the two find ids into the pool at increasing odds by quartile (islands `0..10` ×1, `10..15` ×2, `15..20` ×4), and the last three islands (`isle + 3 >= ISLAND_COUNT`) are `apex`: +25% boss hp/xp, +15% boss damage. **The Archipelago's bosses are not crowns** - they are ordinary named bosses like every zone boss elsewhere, not in `CROWNS`/`CROWN_TARGETS`; nothing here needs the three-places rule above.
