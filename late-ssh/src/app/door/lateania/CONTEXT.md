...
- Last updated: 2026-09-20 (**the Archipelago is a ramp, not a wall, and loot tiers answer to health rather than level.** `extend_archipelago` now authors hp/damage/xp per island directly and `Band::Archipelago`'s tuning row is **1:1**, so the generator's numbers are the fielded ones: regulars Lv80-100, bosses Lv82-100 at one level per island, xp per point of health **1.40** against Kaelmyr's 0.89. The apex isles keep +25% hp/xp and lose their damage bonus. This replaces `tier = isle + 52`, which flattened all 20 islands to Lv100 at up to 3x the last crown's bite; a tier scales hp, damage and xp together, which is exactly why it could not fix the xp complaint without wrecking the levels. Settled by decision: the Archipelago is off-road content that runs *past* Kaethyr Ascendant, who stays the strongest thing on the road. Pinned by `the_archipelago_ramps_past_the_last_crown_instead_of_sitting_flat_at_the_cap` and `the_archipelago_is_the_fastest_ground_in_the_game_to_reach_the_cap_on`, and confirmed against the arena's own boss table: Lv82 Vitreon at 15000hp/348 through to Lv100 The Sundering, Made Flesh at 29912hp/443, one level per island. The ladder crosses the last crown's 397 bite at **island 10** (Your Own Reflection, Wrong, Lv92, 398), so islands 0-9 hit softer than Kaethyr Ascendant and 10-19 harder. Island 0's boss is strictly weaker than him on both axes (15000hp/348 against 24542/397), which is what makes the shallow isles somewhere to grow when he is walling you: the arena shows every class beating him in Kaelmyr-top gear, so the same character clears island 0. Also: a land's loot tier is set against the **health it charges**, never its displayed level (level is a damage reading; health never enters it). Audited every side land against the road's health-per-tier curve: Broceliande and Thornveil were already priced right and were left alone, and the Wildbound's Scorched Flats were paying t=18 for 3960hp regulars and now draw t=25-29 through the new `items::realm_loot`, the shared Frontier→Reaches→Kaelmyr ladder walk. `region_atlas_yardstick` gained median-health and xp-per-health columns, which is what made the audit possible. Before that, 2026-09-18: companions are never released: `pets::Kennel` keeps every owned pet, one per species; a tame never displaces the pet at your heel and only trains Taming when the species is owned; swap at a Stable; see "The kennel" in §7. Stairs on the map are `⇓`/`⇑`/`⇕`. The standing keys show `: waypoint` and `/ warp` instead of say/ranks/leave. Before that, 2026-09-17: player-feedback pass, ported from a sibling standalone deployment: the Shattered Archipelago's tier was raised to `isle + 52` on the premise that `isle + 14` left its islands below Kaelmyr. **That premise was wrong** and the change was reverted on 2026-09-20: the old band row multiplied Archipelago damage by 19/10 and 21/10 against Kaelmyr's 2/3 and 4/5, so `isle + 14` already read Lv69-100, above Kaelmyr. Comparing tier literals across bands is what produced the error. The Archipelago also gained its own 200-item generated catalog (`archipelago_items`/`archipelago_loot`, IDs `5000..5200`, `Band::Archipelago` unchanged) instead of reusing `reaches_loot`, so its mobs and bosses stop handing back Sundered Reaches repeats; boss find-item odds ramp up through the back half/quarter of the 20-island chain, and the last three islands are `apex` fights. Added **Thornveil Falls** (rooms `34000+`, `extend_thornveil`, hung off Broceliande's own deepest chamber), a sixth continent and a new `Band::Thornveil` riding the exact same tuning row as `Band::Kaelmyr` - a parallel endgame track overlapping Kaelmyr's lower tiers, not a crown (not in `CROWNS`/`CROWN_TARGETS`, same as the Archipelago), so Kaelmyr isn't the only late-game road. Arena-verified on 2026-09-20 (`make arena`, 20/20 green), with one limit worth knowing: the arena fights `CROWN_TARGETS`, and neither the Archipelago nor Thornveil holds a crown, so no simulated fight is run against either. What the arena does give for them is the "Every boss as the engine fields it" table in `lateania-arena-extra.md`, which prints their fielded stats, and that is how the Archipelago ladder below was confirmed end to end. Before that, 2026-09-16: the room panel reshaped: a responsive side rail, room-specific actions promoted to the top, the standing-key catalogue cut to a few packed lines pointing at the `?` guide and anchored to the floor of the rail, and the panel finally scrolls with vitals pinned. See "Panels" in §5. Also: the shops scale with the character: each gear shop also stocks its own slots from the generated realm catalogs at `PlayerState::market_tier`, derived from level against `world::CROWNS`' kit rows less `MARKET_LAG_PCT` and capped by the gate titles, so gold is a floor under a straggler and a real late-game sink instead of dead weight after L37. Consumables stay off the market on purpose. See "Items, shops, and rewards" in §7. Before that, 2026-09-15: mounts removed; every companion on one ladder via `PetSource`/`PetSpecies::rung`, the Stable grown to eight pets reaching rung 35, and the tame-50 pile-up spread over taming 46-54 so no beast is dead content. See the Animal Taming section, `FeatureKind::Stable`, and §9.)
- Status: Active
...
| The Shattered Archipelago · 20 islands | 20000+ | Lv80-100 | 20, Lv82-100 |
